...
namespace Lang {{
    ...
    // String resources (en-US used as fallback for missing keys)
    namespace Strings {{
{strings}
    }}

    // Sound resources (en-US used as fallback for missing audio files)
    namespace Sounds {{
{sounds}
    }}
}}
...
def load_base_language(assets_dir):
    """Load the en-US base language data."""
...
def get_sound_files(directory):
    """Return the list of sound files in a directory."""
    if not os.path.exists(directory):
        return []
    return [f for f in os.listdir(directory) if f.endswith('.ogg')]

def generate_header(lang_code, output_path):
    # Derive the project structure from the output path.
    # output_path is typically main/assets/lang_config.h
    ...
    # Build the language JSON file path.
    ...
    # Merge strings: start with en-US, then overlay the selected language.
    ...
    # Collect sound files: start with en-US, then overlay the selected language.
    ...
    # Merge sound file lists: selected language overrides the base language.
    ...
    for file in sorted(all_sound_files):
        ...
        # Prefer the current language sound file, falling back to en-US.
